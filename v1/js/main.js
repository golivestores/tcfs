...
class HeroBanner {
    ...
    init() { this.setupCursor(); this.setupClick(); this.setupPlayPause(); this.setupProgressClicks(); this.startProgress(); const v=this.slides[0].querySelector('video'); if(v)v.play().catch(()=>{}); }
    setupCursor() {
    ...
    setupClick() { this.banner.addEventListener('click', e => { if(e.target.closest('.banner-playpause,.banner-progress,.slide-cta'))return; const mid=this.banner.getBoundingClientRect().left+this.banner.offsetWidth/2; e.clientX>mid?this.goNext():this.goPrev(); }); }
...
class CollectionSection {
    constructor() {
        ...
        window.addEventListener('resize',()=>{this.updateTrackPosition();this.updateArrows()});
    }
...
class PromoCarousel {
    constructor() {
        ...
        this.data = [
            { textBg:'#111', imgBg:"url('https://cdn.shopify.com/s/files/1/0577/1939/0270/files/3efbe62540442d938fcafa2cc5ce56e1.jpg?v=1772781311') center/cover no-repeat",
              label:'會員限定!', labelColor:'#aaa', title:'驚喜回饋金',
              desc:'滿NT$600即享NT$180回饋金。無需累點，輕鬆領取。', btn:'立即選購' },
            { textBg:'#222', imgBg:"url('https://cdn.shopify.com/s/files/1/0577/1939/0270/files/951d938b2bc8263f1d5e15be9404268e.jpg?v=1772781311') center/cover no-repeat",
              label:'限時優惠!', labelColor:'#999', title:'滿額贈正裝好禮',
              desc:'單筆消費滿NT$1,200即享免運。數量有限，送完為止。', btn:'立即選購' },
            { textBg:'#333', imgBg:"url('https://cdn.shopify.com/s/files/1/0577/1939/0270/files/bc1bf207ee25153b7ee60e01a978458e.jpg?v=1772781311') center/cover no-repeat",
              label:'新品!', labelColor:'#bbb', title:'全新光澤系列',
              desc:'柔焦光感妝效，讓肌膚煥發自然透亮光彩。', btn:'立即探索' },
            { textBg:'#1a1a1a', imgBg:"url('https://cdn.shopify.com/s/files/1/0577/1939/0270/files/397de29c3faac41dc6b24e3772dc5b81.jpg?v=1772783471') center/cover no-repeat",
              label:'春季限定', labelColor:'#888', title:'春日護膚套裝',
              desc:'全套護膚流程，為肌膚注入春日活力與水潤。', btn:'立即選購' },
            { textBg:'#2a2a2a', imgBg:"url('https://cdn.shopify.com/s/files/1/0577/1939/0270/files/397de29c3faac41dc6b24e3772dc5b81.jpg?v=1772783471') center/cover no-repeat",
              label:'限量發售', labelColor:'#aaa', title:'限量聯名系列',
              desc:'獨家設計包裝，收藏級美妝單品，錯過不再。', btn:'立即選購' }
        ];

        this.setSize = this.data.length;
        this.displayIdx = this.setSize;
        this.gap = 12;
        this.autoTimer = null;
        this.autoDelay = 5000;
        this.jumping = false;
    ...
    getWidths() {
        const cw = this.track.parentElement.offsetWidth;
        const w = window.innerWidth;
        if (w <= 480) return { active: cw, inactive: cw, container: cw };
        if (w <= 768) return { active: Math.round(cw * 0.58), inactive: Math.round(cw * 0.32), container: cw };
        return { active: Math.round(cw * 0.55), inactive: Math.round(cw * 0.195), container: cw };
    }

    render() {
        const tripled = [...this.data, ...this.data, ...this.data];
        this.track.innerHTML = tripled.map((c, i) =>
            `<a href="#" class="promo-slide" data-display="${i}" data-real="${i % this.setSize}">
                <div class="promo-slide-text" style="background:${c.textBg}">
                    ${c.label ? `<span class="pc-label" style="color:${c.labelColor || '#fff'}">${c.label}</span>` : ''}
                    <h3 class="pc-title">${c.title}</h3>
                    <p class="pc-desc">${c.desc}</p>
                    <span class="pc-btn">${c.btn}</span>
                </div>
                <div class="promo-slide-img"><div class="promo-slide-img-bg" style="background:${c.imgBg}"></div></div>
            </a>`
        ).join('');
        this.slideEls = [...this.track.querySelectorAll('.promo-slide')];
        this.markActive();
    }

    markActive() {
        this.slideEls.forEach((el, i) => el.classList.toggle('active', i === this.displayIdx));
    }
    ...
    updateLayout(animate = true) {
        ...
        if (!animate) {
            this.track.style.transition = 'none';
            this.slideEls.forEach(el => el.style.transition = 'none');
        }
        this.slideEls.forEach((el, i) => {
            el.style.width = (i === this.displayIdx ? w.active : w.inactive) + 'px';
        });
        const offset = this.displayIdx * (w.inactive + this.gap);
        this.track.style.transform = `translateX(-${offset}px)`;
        if (!animate) {
            void this.track.offsetHeight;
            this.track.style.removeProperty('transition');
            this.slideEls.forEach(el => el.style.removeProperty('transition'));
        }
    }

    goTo(direction) {
        if (this.jumping) return;
        this.displayIdx += direction;
        this.markActive();
        this.updateLayout(true);

        const realIdx = ((this.displayIdx % this.setSize) + this.setSize) % this.setSize;
        document.querySelectorAll('.promo-dot').forEach((d, i) =>
            d.classList.toggle('active', i === realIdx));
        this.resetAuto();

        const lo = this.setSize;
        const hi = this.setSize * 2 - 1;
        if (this.displayIdx < lo || this.displayIdx > hi) {
            this.jumping = true;
            setTimeout(() => {
                this.displayIdx = lo + realIdx;
                this.markActive();
                this.updateLayout(false);
                this.jumping = false;
            }, 750);
    ...
    goToReal(realIdx) {
        if (this.jumping) return;
        this.displayIdx = this.setSize + realIdx;
        this.markActive();
        this.updateLayout(true);
        document.querySelectorAll('.promo-dot').forEach((d, i) =>
            d.classList.toggle('active', i === realIdx));
    ...
    renderDots() {
        const c = document.getElementById('promoDots');
        c.innerHTML = Array.from({ length: this.setSize }, (_, i) =>
            `<button class="promo-dot${i === 0 ? ' active' : ''}" data-p="${i}"></button>`
        ).join('');
        c.querySelectorAll('.promo-dot').forEach(d =>
            d.addEventListener('click', () => this.goToReal(parseInt(d.dataset.p)))
        );
    }

    setupNav() {
        document.getElementById('promoPrev').addEventListener('click', () => this.goTo(-1));
        document.getElementById('promoNext').addEventListener('click', () => this.goTo(1));
    }

    startAuto() {
        this.autoTimer = setInterval(() => this.goTo(1), this.autoDelay);
    }
...
/* ============================================
   Beauty From Every Angle (滾動視差)
   ============================================ */
class BeautyAngleSection {
    constructor() {
        this.section = document.getElementById('beautyAngle');
        if (!this.section) return;
        this.images = [...this.section.querySelectorAll('.ba-img')];
        this.body = this.section.querySelector('.ba-body');

        // Start offsets (vw, vh): translations to cluster images to center at progress 0
        // img1 (top-left → center): push right and down
        // img2 (top-right → center): push left and down
        // img3 (bottom-left → center): push right and up
        // img4 (bottom-right → center): push left and up
        this.offsets = [
            [26, 26],
            [-32, 26],
            [28, -14],
    ...
    update() {
        ...
        const scrollRange = this.section.offsetHeight - window.innerHeight;
        if (scrollRange <= 0) return;

        const progress = Math.max(0, Math.min(1, -rect.top / scrollRange));
        const eased = this.easeOutQuad(progress);

        // Update image transforms — spread from center to final positions
        this.images.forEach((img, i) => {
            const [sx, sy] = this.offsets[i];
            const tx = sx * (1 - eased);
            const ty = sy * (1 - eased);
            const scale = 1.04 - 0.04 * eased;
            img.style.transform = `translate(${tx}vw, ${ty}vh) scale(${scale})`;
        });

        // Fade in paragraph + button
        const textP = Math.max(0, Math.min(1, (progress - 0.2) / 0.45));
        this.body.style.opacity = textP;
    }
...
class RoutineSection {
    constructor() {
        this.list = document.getElementById('routineList');
        if (!this.list) return;

        this.steps = [
            { num:'01', name:'水潤打底', product:'水潤打底乳', desc:'舒緩保濕妝前乳', price:34,
              thumbBg:"url('https://cdn.shopify.com/s/files/1/0577/1939/0270/files/77f0ef16e0fbc52baa3dea6f4a2e4a0e.jpg?v=1772784822') center/contain no-repeat",
              video:'https://cdn.shopify.com/videos/c/o/v/9718c4eaf6164822966d4be416d4d617.mp4',
              link:'#product-primer' },
            { num:'02', name:'精華滴注', product:'亮膚精華液', desc:'亮白保濕精華，打造水光肌膚', price:36,
              thumbBg:"url('https://cdn.shopify.com/s/files/1/0577/1939/0270/files/d7d786ffc26a7e958be6a71929934d0b.jpg?v=1772784822') center/contain no-repeat",
              video:'https://cdn.shopify.com/videos/c/o/v/7dbf1928381e4caa823c53539438cafd.mp4',
              link:'#product-serum' },
            { num:'03', name:'霜潤滋養', product:'滋潤面霜', desc:'乾敏肌專用保濕面霜', price:38,
              thumbBg:"url('https://cdn.shopify.com/s/files/1/0577/1939/0270/files/484291c3591841355507e3c3352a3e2b.jpg?v=1772784822') center/contain no-repeat",
              video:'https://cdn.shopify.com/videos/c/o/v/75aee86f646347798afd6142073f064c.mp4',
              link:'#product-cream' },
            { num:'04', name:'光澤妝前', product:'光澤妝前乳', desc:'光澤黏附妝前乳', price:28,
              thumbBg:"url('https://cdn.shopify.com/s/files/1/0577/1939/0270/files/2df6780054db4d9b458828e39609f31c.jpg?v=1772784822') center/contain no-repeat",
              video:'https://cdn.shopify.com/videos/c/o/v/1ab3595bcfc44e1c9e6d837f631d7ea1.mp4',
              link:'#product-glow-primer' },
            { num:'05', name:'立體修容', product:'修容盤', desc:'藝術課堂修容盤', price:32,
              thumbBg:"url('https://cdn.shopify.com/s/files/1/0577/1939/0270/files/111b02976f89989ea9015e8eb99bb940.jpg?v=1772784822') center/contain no-repeat",
              video:'https://cdn.shopify.com/videos/c/o/v/b8eff60a7b6b493882f5b11e01de3c16.mp4',
              link:'#product-contour' },
            { num:'06', name:'持久定妝', product:'定妝噴霧', desc:'持久定妝噴霧', price:24,
              thumbBg:"url('https://cdn.shopify.com/s/files/1/0577/1939/0270/files/076dcebd67088d85d2eb403f65020479.jpg?v=1772784822') center/contain no-repeat",
              video:'https://cdn.shopify.com/videos/c/o/v/d1dbb544c89d43c2ac190cc38ce600f7.mp4',
              link:'#product-setting-spray' }
        ];

        this.activeIdx = 0;
        this.arrow = document.getElementById('routineArrow');
        this.renderSteps();
        this.renderVideos();
        this.setupHover();
        this.updateProdCard();
        requestAnimationFrame(() => requestAnimationFrame(() => this.updateArrow()));
        window.addEventListener('resize', () => this.updateArrow());
    }

    renderSteps() {
        this.list.innerHTML = this.steps.map((s, i) => `
            <div class="routine-step${i === 0 ? ' active' : ''}" data-index="${i}">
                <span class="rs-num">${s.num}</span>
                <span class="rs-name">${s.name}</span>
                <div class="rs-thumb" style="background:${s.thumbBg}"></div>
            </div>`).join('');
        this.stepEls = [...this.list.querySelectorAll('.routine-step')];
    }

    renderVideos() {
        const container = document.getElementById('routineVids');
        if (!container) return;
        container.innerHTML = this.steps.map((s, i) =>
            `<div class="routine-vid-bg${i === 0 ? ' active' : ''}" data-index="${i}">
                <a href="${s.link}" style="position:absolute;inset:0;display:block">
                    <video src="${s.video}" muted loop playsinline preload="metadata" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;pointer-events:none"></video>
                </a>
            </div>`
        ).join('');
        this.vidBgs = [...container.querySelectorAll('.routine-vid-bg')];
        const firstVid = this.vidBgs[0].querySelector('video');
    ...
    setupHover() {
        this.stepEls.forEach(el => {
            el.addEventListener('mouseenter', () => {
                ...
                const prevVid = this.vidBgs[this.activeIdx].querySelector('video');
                if (prevVid) prevVid.pause();
                this.activeIdx = idx;
                this.stepEls.forEach((s, i) => s.classList.toggle('active', i === idx));
                this.vidBgs.forEach((v, i) => v.classList.toggle('active', i === idx));
                const curVid = this.vidBgs[idx].querySelector('video');
                if (curVid) { curVid.currentTime = 0; curVid.play().catch(() => {}); }
                this.updateArrow();
                this.updateProdCard();
            });
        });
    ...
    updateArrow() {
        const step = this.stepEls[this.activeIdx];
        if (!step || !this.arrow) return;
        const parentRect = this.arrow.parentElement.getBoundingClientRect();
        const stepRect = step.getBoundingClientRect();
        this.arrow.style.top = (stepRect.top - parentRect.top + stepRect.height / 2) + 'px';
    }

    updateProdCard() {
        const s = this.steps[this.activeIdx];
        const card = document.getElementById('routineProdCard');
        if (!card) return;
        card.style.opacity = '0';
        setTimeout(() => {
            document.getElementById('rpcName').textContent = s.product;
            document.getElementById('rpcDesc').textContent = s.desc;
            document.getElementById('rpcAdd').textContent = `加入購物車 – $${s.price}`;
            document.getElementById('rpcThumb').style.background = s.thumbBg;
